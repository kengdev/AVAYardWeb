...
namespace AVAYardWeb.Controllers
{
    public class OrderPaymentController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> GetCost(string container, string code)
        {
            var containerSize = await db.TransContainerSizes.Where(w => w.ContainerSizeCode == container).FirstOrDefaultAsync();
            var costData = await db.TaxTotalCosts.Where(w => w.TaxId == code && w.IsEnabled == true).FirstOrDefaultAsync();

            if(containerSize.ContainerTypeCode == "S")
            {
                return Json(costData.SizeShortCost);
            }
            else
            {
                return Json(costData.SizeLongCost);
            }
        }
